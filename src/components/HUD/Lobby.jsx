...
import { CheckCircle, XCircle, Shield, Volume2, VolumeX, Users } from 'lucide-react';
...
export const Lobby = () => {
  ...
  return (
    <div
      ...
      style={{ background: 'radial-gradient(circle at center, #0d1117 0%, #050a10 100%)', pointerEvents: 'auto' }}
    >
      <div
        ...
        style={{
          ...
          padding: '34px 40px',
          borderRadius: 10,
          background: 'linear-gradient(135deg, rgba(13,17,23,0.95) 0%, rgba(8,12,18,0.97) 100%)',
          zIndex: 2,
        }}
      >
        ...
        <div className="lobby-header" style={{ marginBottom: 24, textAlign: 'center' }}>
          <Users size={28} color="var(--accent-blue)" style={{ marginBottom: 6 }} />
          <h2
            style={{
          ...
          <div className="war-divider"><span>Players · Ready Check</span></div>
          ...
          >
            ...
            <span style={{ color: 'var(--accent-green)' }}>{readyCount} Ready</span>
          </div>
        </div>
                ...
                {p ? (
                  <>
                    ...
                    <div className="war-slot__avatar">
                      <Shield size={22} color={p.isReady ? 'var(--accent-green)' : 'var(--accent-blue)'} />
                    </div>
                    <div className="war-slot__info">
                      <span className="war-slot__name">
                        {p.name}
                        {isMe && (
                          <span style={{ marginLeft: 6, fontSize: '0.65rem', color: 'var(--accent-yellow)', letterSpacing: 2 }}>
                            (YOU)
                          </span>
                        )}
                      </span>
                      <span className="war-slot__country">{isMe ? p.country : '---'}</span>
                    </div>
                    <div className="war-slot__rank">{p.isReady ? 'READY' : 'NOT READY'}</div>
                    {p.isReady
        ...
        <div>
          <label className={`war-toggle${isReady ? ' armed' : ''}`} style={{ width: '100%' }}>
            <input type="checkbox" checked={isReady} onChange={toggleReady} style={{ display: 'none' }} />
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
              {[
                { label: 'MARKET', color: 'var(--accent-red)', border: 'rgba(248,81,73,0.25)', desc: 'Buy weapons during the Prep phase.' },
                { label: 'R&D', color: 'var(--accent-yellow)', border: 'rgba(227,179,65,0.3)', desc: 'Research cheaper gear. 40% risk of failure.' },
                { label: 'ATTACK', color: 'var(--accent-green)', border: 'rgba(63,185,80,0.3)', desc: 'Drag weapons onto enemy countries to attack.' },
              ].map((item) => (
                <div key={item.label} style={{
                  ...
                  <strong style={{ color: item.color, letterSpacing: 1 }}>{item.label}</strong>
                  <br />
      ...
      <div className="war-status-strip">
        ...
        <div className="war-status-strip__cell">
          <span className="war-led amber" /> {players.length}/6 Players
        </div>
        <div className="war-status-strip__cell">
          <span className="war-led red" /> {readyCount} Ready
        </div>
      </div>
    </div>
  );
